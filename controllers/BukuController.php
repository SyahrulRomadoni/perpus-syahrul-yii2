...
class BukuController extends Controller
{
    ...
    public function behaviors()
    {
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['index', 'update', 'create', 'daftar-buku-pdf', 'daftar-buku-excel'],
    ...
    public function actionDaftarBukuWord()
    {
        ...
        $filename = time() . '_Daftar-Buku.docx';
    ...
    {
        $searchModel = new Buku();
        $searchModel = $searchModel->find()->all();
        
        $content = $this->renderPartial('@app/template/daftarbukupdf',['model' => $searchModel]);
...
CSS;
        ...
        $pdf->filename = $time . "_Daftar-Buku.pdf";

        // return the pdf output as per the destination setting
        return $pdf->render();
    }

    public function actionDaftarBukuExcel()
    {
        ...
        $spreadsheet->getActiveSheet()->setCellValue('A1', 'Daftar Buku');

        $spreadsheet->getActiveSheet()->getStyle('A3:F3')->getFill()->setFillType(Fill::FILL_SOLID);
        $spreadsheet->getActiveSheet()->getStyle('A3:F3')->getFill()->getStartColor()->setARGB('d8d8d8');
        ...
        foreach($searchModel->find()->all() as $buku){
            $row++;
            $sheet->setCellValue('A' . $row, $i);
            $sheet->setCellValue('B' . $row, $buku->nama);
            $sheet->setCellValue('C' . $row, $buku->tahun_terbit);
            $sheet->setCellValue('D' . $row, $buku->penulis->nama);
            $sheet->setCellValue('E' . $row, $buku->penerbit->nama);
            $sheet->setCellValue('F' . $row, $buku->kategori->nama);
        ...
        $filename = time() . '_Daftar-Buku.xlsx';
        $path = 'document/' . $filename;
